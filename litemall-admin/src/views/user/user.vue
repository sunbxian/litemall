<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" :element-loading-text="$t('app.message.list_loading')" border fit highlight-current-row>
      ...
      <el-table-column align="center" :label="$t('user_user.table.role')" prop="role">
        <template slot-scope="scope">
          <el-tag>{{ roleDic[scope.row.role] }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="userDialogVisible" :title="$t('user_user.dialog.edit')">
      <el-form ref="userDetail" :model="userDetail" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item :label="$t('user_user.form.role')" prop="status">
          <el-select v-model="userDetail.role" :placeholder="$t('user_user.placeholder.role')">
            <el-option
              v-for="(label, value) in roleDic"
              :key="value"
              :label="label"
              :value="value"
            />
